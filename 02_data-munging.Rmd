...
UC = c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")
...
plotly::ggplotly(
  ggplot(park_percentage) +
    geom_col(aes(x = Year, y = Percent, color = Park)) +
    theme_bw(base_size=10))  
...
```{r}
average<-park_percentage%>%
  group_by(Park)%>%
  mutate(average = mean(Percent))

plotly::ggplotly(
  ggplot(average)+
  geom_point(aes(x = Park, y = average)))
```

Great Smokey Mountains has the highest average visitation and Zion has the least.
