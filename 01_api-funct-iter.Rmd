...
```{r}
raw_ROMO <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
...
parkwide_visitation<- function(){
  ...
  raw_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1997")
...
```{r}
unit_visitation<-function(UC = "MACA", startMonth = 1, startYear = 2010, endMonth = 12, endYear = 2022){
  
  raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", UC, "&startMonth=", startMonth, "&startYear=", startYear, "&endMonth=", endMonth, "&endYear=", endYear))
unpacked_data <- httr::content(raw_data, as = "text", encoding = "UTF-8") 
final_data <- jsonlite::fromJSON(unpacked_data)
...
```{r}
ROMO<-unit_visitation(UC = "ROMO", startYear = 1990, endYear = 2021)
EVER<-unit_visitation(UC = "ROMO", startYear = 1990, endYear = 2021)
THRO<-unit_visitation(UC = "THRO", startYear = 1990, endYear = 2021)
```
...
```{r}
unit_visitation<-function(UC = "MACA", startMonth = 1, startYear = 1980, endMonth = 12, endYear = 2021){
...
ROMO<-unit_visitation(UC = "ROMO")
EVER<-unit_visitation(UC = "EVER")
THRO<-unit_visitation(UC = "THRO")
```
...
```{r}
UC = c("ROMO", "EVER", "THRO")
output_parks <- vector("list", length = length(UC))

for (x in 1:length(UC)) {
 
  output_parks[[x]]<-
    unit_visitation(UC = UC[x])
}

parks_data<-dplyr::bind_rows(output_parks)
...
```{r}
output_parks2 <- UC %>% 
  map(~ unit_visitation(UC = .))

multi_parks <- bind_rows(output_parks2)
